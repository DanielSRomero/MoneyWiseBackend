...
class Utils {
    companion object{
        fun createBase64ToImg(img: String, userName: String) : String?{
            ...
            return if (result != null) {
                ...
                try {
                    ...
                    val path : String = ApplicationContext.context.environment.config.property("ktor.path.images").getString() + "/$userName"
                    val dir = File(path)
                    if (dir.isDirectory){
                        val nFile: String = path+"/"
                        val nameFile = userName+"_${SimpleDateFormat("yyyyMMdd_HHmmss").format(Date())}.$ext"
        ...
        fun deleteImage(userName: String, name: String):Boolean{
            try{
        ...
        fun deleteDirectory(userName: String) : Boolean{
            try{
                val path = ApplicationContext.context.environment.config.property("ktor.path.images").getString()+"/$userName"
                val dir = File(path)
                if (dir.exists()){
